...
func find_nodes():
	...
	if body == null:
		for child in get_children():
			if child is Node3D and child.name not in ["NavigationAgent3D", "HealthLabel", "CollisionShape3D", "DetectionArea"]:
				body = child
				break
...
func _on_detection_area_body_entered(body_node: Node3D):
	if body_node.is_in_group("player"):
		player_in_range = true
		player = body_node


func _on_detection_area_body_exited(body_node: Node3D):
	if body_node.is_in_group("player"):
...
func _physics_process(delta):
	...
	if not should_chase and detection_area == null:
		...
		should_chase = distance <= detection_range
...
func play_anim(anim_name: String):
	...
	if is_attacking and anim_name != "Grunt/Attack" and anim_name != "Grunt/Death":
		return
...
func update_health_display():
	...
	if health_label:
		var current_hp = int(max(current_health, 0))
		var max_hp = int(max_health)
...
func die():
	...
	if anim_player and anim_player.has_animation("Grunt/Death"):
...
func drop_loot():
	...
	if coin_scene:
		...
		for i in range(coin_count):
			...
			coin.value = randi_range(3, 8)
	
	if health_scene and randf() < 0.4:
		var health = health_scene.instantiate()
		get_tree().root.add_child(health)
		health.global_position = drop_position + Vector3(0, 0.5, 0)
		health.heal_amount = 25
